...
class DocumentTypeCreateView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if request.is_ajax():
                ...
                if form.is_valid():
                    form.save()
                    message = f'Tipo de Documento registrada correctamente'
                    error = 'No han ocurrido errores'
                    response = JsonResponse({'message': message, 'error': error})
                    response.status_code = 201
                    return response
                else:
                    message = f'Tipo de Documento no se pudo registrar!'
                    error = form.errors
                    response = JsonResponse({'message': message, 'error': error})
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Creación de Tipo de Documento'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if request.is_ajax():
                ...
                if form.is_valid():
                    form.save()
                    message = f'Tipo de Documento actualizado correctamente'
                    error = 'No hay error'
                    response = JsonResponse({'message': message, 'error': error})
                    response.status_code = 201
                    return response
                else:
                    message = f'Tipo de Documento no se pudo actualizar!'
                    error = form.errors
                    response = JsonResponse({'message': message, 'error': error})
                    response.status_code = 400
                    return response
...
class DocumentTypeDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            obj.state = False
            obj.save()
            message = f'Tipo de Documento eliminada correctamente!'
