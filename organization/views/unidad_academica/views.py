...
class AcademicUnitListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pageview'] = 'Unidad Académica'
        context['object_list'] = AcademicUnit.objects.filter(state=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if request.is_ajax():
                ...
                if form.is_valid():
                    form.save()
                    message = f'Unidad Académica registrada correctamente'
                    error = 'No han ocurrido errores'
                    response = JsonResponse({'message': message, 'error': error})
                    response.status_code = 201
                    return response
                else:
                    message = f'Unidad Académica no se pudo registrar!'
                    error = form.errors
                    response = JsonResponse({'message': message, 'error': error})
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if request.is_ajax():
                ...
                if form.is_valid():
                    form.save()
                    message = f'Unidad Académica actualizado correctamente'
                    error = 'No hay error'
                    response = JsonResponse({'message': message, 'error': error})
                    response.status_code = 201
                    return response
                else:
                    message = f'Unidad Académica no se pudo actualizar!'
                    error = form.errors
                    response = JsonResponse({'message': message, 'error': error})
                    response.status_code = 400
                    return response
...
class AcademicUnitDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            obj.state = False
            obj.save()
            message = f'Unidad Académica eliminada correctamente!'
